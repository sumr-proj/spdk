 ...
 */
static int addr_cmp(struct malloc_write_request *c1, struct malloc_write_request *c2);
 ...
 */
static int interception_malloc_write_request(struct spdk_bdev_io *bdev_io,
					     struct malloc_channel *mch);

static int malloc_verify_pi(struct spdk_bdev_io *bdev_io)
{
	...
	rc = spdk_dif_ctx_init(&dif_ctx, bdev->blocklen, bdev->md_len, bdev->md_interleave,
			       bdev->dif_is_head_of_md, bdev->dif_type, bdev->dif_check_flags,
			       bdev_io->u.bdev.offset_blocks & 0xFFFFFFFF, 0xFFFF, 0, 0, 0,
			       &dif_opts);
	if (rc != 0) {
	...
	if (spdk_bdev_is_md_interleaved(bdev)) {
		...
		rc = spdk_dix_verify(bdev_io->u.bdev.iovs, bdev_io->u.bdev.iovcnt, &md_iov,
				     bdev_io->u.bdev.num_blocks, &dif_ctx, &err_blk);
	}
	...
	if (rc != 0) {
		SPDK_ERRLOG("DIF/DIX verify failed: lba %" PRIu64 ", num_blocks %" PRIu64 ", "
			    ...
			    err_blk.err_type, err_blk.expected, err_blk.actual, err_blk.err_offset);
	}

	return rc;
}

static void malloc_done(void *ref, int status)
{
...
static void malloc_complete_task(struct malloc_task *task, struct malloc_channel *mch,
				 enum spdk_bdev_io_status status)
{
	task->status = status;
...
static int bdev_malloc_get_ctx_size(void)
{
...
static void malloc_disk_free(struct malloc_disk *malloc_disk)
{
	if (!malloc_disk) {
		return;
	}
...
static int bdev_malloc_destruct(void *ctx)
{
...
static int bdev_malloc_check_iov_len(struct iovec *iovs, int iovcnt, size_t nbytes)
{
	int i;
...
static void malloc_sequence_fail(struct malloc_task *task, int status)
{
	struct spdk_bdev_io *bdev_io = spdk_bdev_io_from_ctx(task);
...
static void malloc_sequence_done(void *ctx, int status)
{
...
static int bdev_malloc_readv(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
			     struct malloc_task *task, struct spdk_bdev_io *bdev_io)
{
	...
	SPDK_DEBUGLOG(bdev_malloc, "read %zu bytes from offset %#" PRIx64 ", iovcnt=%d\n", len,
		      offset, bdev_io->u.bdev.iovcnt);

	task->num_outstanding++;
	res = spdk_accel_append_copy(&bdev_io->u.bdev.accel_sequence, ch, bdev_io->u.bdev.iovs,
				     bdev_io->u.bdev.iovcnt, bdev_io->u.bdev.memory_domain,
				     bdev_io->u.bdev.memory_domain_ctx, &task->iov, 1, NULL, NULL,
				     0, NULL, NULL);
...
static void bdev_malloc_writev(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
			       struct malloc_task *task, struct spdk_bdev_io *bdev_io)
{
	uint64_t len, offset, md_offset;
	int res = 0;
...
static int bdev_malloc_unmap(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
			     struct malloc_task *task, uint64_t offset, uint64_t byte_count)
{
	task->status = SPDK_BDEV_IO_STATUS_SUCCESS;
	task->num_outstanding = 1;

	return spdk_accel_submit_fill(ch, mdisk->malloc_buf + offset, 0, byte_count, 0, malloc_done,
				      task);
}

static void bdev_malloc_copy(struct malloc_disk *mdisk, struct spdk_io_channel *ch,
			     struct malloc_task *task, uint64_t dst_offset, uint64_t src_offset,
			     size_t len)
{
	int64_t res = 0;
	...
	SPDK_DEBUGLOG(bdev_malloc,
		      "Copy %zu bytes from offset %#" PRIx64 " to offset %#" PRIx64 "\n", len,
					     ...
					     struct malloc_channel *mch)
{
	...
	SPDK_DEBUGLOG(bdev_malloc,
		      "One node with address - %ld, insert into the tree with size - %ld\n",
...
static bool bdev_malloc_io_type_supported(void *ctx, enum spdk_bdev_io_type io_type)
{
	switch (io_type) {
	case SPDK_BDEV_IO_TYPE_READ:
	case SPDK_BDEV_IO_TYPE_WRITE:
...
static struct spdk_io_channel *bdev_malloc_get_io_channel(void *ctx)
{
...
static int bdev_malloc_get_memory_domains(void *ctx, struct spdk_memory_domain **domains,
					  int array_size)
{
	struct malloc_disk *malloc_disk = ctx;
...
static bool bdev_malloc_accel_sequence_supported(void *ctx, enum spdk_bdev_io_type type)
{
	struct malloc_disk *malloc_disk = ctx;
...
static int malloc_disk_setup_pi(struct malloc_disk *mdisk)
{
...
int create_malloc_disk(struct spdk_bdev **bdev, const struct malloc_bdev_opts *opts)
{
   ...
   */
	...
	if (!opts->md_interleave && opts->md_size != 0) {
		mdisk->malloc_md_buf = spdk_zmalloc(opts->num_blocks * opts->md_size,
						    2 * 1024 * 1024, NULL, SPDK_ENV_LCORE_ID_ANY,
						    SPDK_MALLOC_DMA);
...
static int poller_of_tree(struct malloc_channel *ch)
{
...
static int malloc_completion_poller(void *ctx)
{
...
static int malloc_create_channel_cb(void *io_device, void *ctx)
{
	struct malloc_channel *ch = ctx;
...
static void malloc_destroy_channel_cb(void *io_device, void *ctx)
{
	struct malloc_channel *ch = ctx;
